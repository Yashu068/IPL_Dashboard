...
export default function FantasyBuilder(){
  const { players, fantasyTeam, addPlayerToFantasy, removePlayerFromFantasy, clearFantasy } = useContext(AppContext)
    ...
    <div>
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <h3 className="font-semibold mb-2">Selected Team ({fantasyTeam.length}/11)</h3>
            <Droppable droppableId="selected">
              {(provided)=> (
                <div ref={provided.innerRef} {...provided.droppableProps} className="p-2 border rounded min-h-[200px]">
                  {fantasyTeam.map((p, idx)=> (
                    <div key={p.id} className="p-2 mb-2 border rounded flex justify-between items-center">
                      <div>
                        <div className="font-medium">{p.name}</div>
                        <div className="text-xs text-muted">{p.team} • {p.role}</div>
                      </div>
            ...
            <div className="mt-3">
              <button className="px-4 py-2 bg-sporty text-white rounded" onClick={()=>alert('Saved to localStorage (auto)')}>Save Team</button>
            </div>
          </div>
        </div>
      </DragDropContext>
